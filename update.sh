...
_boilerplate_config_entry() {
  ...
  jq -ec --arg name "${name}" '
    .boilerplates[]
    | select(.name == $name)
  ' "${IMAGES_REPO_ROOT}/boilerplates.json"
}
...
_boilerplate_update_failed() {
  ...
  slug=$(jq -r --arg name "${name}" \
    '.boilerplates[] | select(.name == $name) | .repo' \
    "${IMAGES_REPO_ROOT}/boilerplates.json" 2>/dev/null || true)
  slug="${slug:-wodby/${name}}"
...
update_boilerplate_dependencies() (
  ...
  entry=$(_boilerplate_config_entry "${name}") || {
    echo >&2 "Unknown boilerplate: ${name}"
    return 1
  }
  ...
  work_root="${IMAGES_REPO_ROOT}/.boilerplate-work"
  mkdir -p "${work_root}"
  repo_dir=$(mktemp -d "${work_root}/${name}.XXXXXX")
  trap 'rm -rf "${repo_dir}"' EXIT

  relative_repo_dir="${repo_dir#"${IMAGES_REPO_ROOT}/"}"
  host_repo_dir="${IMAGES_HOST_ROOT}/${relative_repo_dir}"
  ...
  mapfile -t changed_files < <(_boilerplate_changed_files "${repo_dir}")
  ...
  _git_commit "${repo_dir}" "Update dependencies" 0
